use super::Store;
use crate::{
    api::{AddMembersRequest, AddMembersResponse, DeleteMembersRequest, DeleteMembersResponse},
    core::Error,
    store::MemberInfo,
};
...
impl Store {
    ...
    pub async fn add_members(&self, arg: &AddMembersRequest) -> Result<AddMembersResponse, Error> {
        let room_ids = vec![arg.room_id; arg.member_ids.len()];

        let mid = sqlx::query_scalar!(
            r#"
                INSERT INTO members (room_id, member_id)
    ...
    ) -> Result<DeleteMembersResponse, Error> {
